...
- [done] Report unrecognised review policy keys as a doctor finding, recovering what dropping DisallowUnknownFields gave up. Delivered by `pose-doctor-reports-unread-policy-keys`: `review.policy-keys` warns on any top-level key the engine does not model, derived from the struct's own json tags. (owner:unowned crit:medium review:2026-11-08)
- [done] Say in the release notes that adopting a contract makes the instance unreadable to engines predating it, since no encoding avoids that. Done in `pose-release-notes-name-contract-adoption`: the notes carry a Compatibility section built from the contract registry, so the release that introduces a contract names it without anyone remembering to, stating the boundary each contract actually has — a top-level key refuses engines before 2.0.2, an id in `contract_adoptions` does not — and a contract added without its introducing release fails a test. (owner:unowned crit:medium review:2026-10-08)
- [done] Run the previous release's binary against a policy this one writes, as a test rather than by hand. Done in `pose-release-boundary-rehearsal`: the previous release is built from its own tag and probed with `review-plan`, which reaches the loader the by-hand `check --strict` never did — that wrong probe is what produced the retracted claim. (owner:unowned crit:medium review:2026-12-08)
