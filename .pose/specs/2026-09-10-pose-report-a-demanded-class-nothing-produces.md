...
- [done] Register checks emitting `a11y` and `e2e` in this repository, or reconcile the profiles that demand them — the new check reports both, and both are real. Neither, and the report was wrong: `evidence_classes` is a disjunction, so every criterion listing `e2e` also accepted `integration` or `unit`, both produced here; and `a11y` belongs to `frontend-review`, selected by `languages: [javascript, typescript]`, which no component here has. Corrected in `pose-class-producers-reads-the-disjunction`. — owner:@pose-maintainers crit:medium review:2026-12-10
