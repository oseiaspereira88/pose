...
- [open] Decide, per field, whether verification should read `allow_criterion_reuse`, `require_signed_attestations`, `reviewer_independence`, `accepted_risk_severities`, `allow_approved_with_reservations` and `component_aware` from the sealed bundle or from live policy — tightening a signing requirement probably should reach an old bundle, and reusing a criterion probably should not — owner:unowned crit:medium review:2026-12-09
