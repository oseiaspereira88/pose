...
- [done] Cover the remaining command surfaces the audit found at zero: review-check, history-check, docs-sync, docs-review, contribute-submit, roadmap-check and the spec-readiness helpers in check.go. Done in `pose-remaining-command-surfaces-coverage`: all twelve off zero, `internal/cli` 71.2% to 72.9%, and the roadmap-check fixture surfaced an early return that makes the gate report zero criteria and pass — recorded there as crit:high — owner:unowned crit:medium review:2026-12-09
