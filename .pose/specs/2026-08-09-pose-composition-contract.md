...
- [done] `EXPOSE` is the source: the contract already derives from it, and `TestDockerfilePortIsSingleSourced` now fails when the same Dockerfile's `ENV *_ADDR` default disagrees — proven by setting one to :9000 against an EXPOSE of 8799. The compose mapping remains the consumer's copy, which is what the contract exists to let it stop maintaining. Original item: the internal port is declared in the Dockerfile's `ENV` and `EXPOSE`, in the compose mapping, and would be declared a fourth time by the contract. (owner:@pose-maintainers crit:low review:2026-11-13)
