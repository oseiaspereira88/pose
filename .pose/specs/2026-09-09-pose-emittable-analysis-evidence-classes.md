...
- [done] Report when a stack has no check emitting a class its selected profiles demand; the vocabulary check is global, so moving `go vet` off `build` left the Go stack with no `build` producer and nothing said so. Done in `pose-report-a-demanded-class-nothing-produces`, per repository rather than per stack: resolving which profile governs which stack is the planner's work, and a per-stack version would report a Go module for not emitting `a11y`. The harm the follow-up describes is visible without that mapping — and the check has two real findings here on its first run, `a11y` and `e2e`. (owner:unowned crit:medium review:2026-12-09)
