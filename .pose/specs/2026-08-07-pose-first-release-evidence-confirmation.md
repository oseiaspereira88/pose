...
- [done] Overtaken by real cuts: v0.20.0 through v0.20.3 all signed, and v0.20.3 was independently verified against the published artifact (`pose release status` projects `verified`). A rehearsal exists to derisk the first real run; four have now happened, so dispatching one would prove less than what is already recorded. Original item: R3: dispatch the Release workflow manually to complete the signing rehearsal, then record sign+verify in this spec. (owner:@pose-maintainers crit:high review:2026-08-21)
- [done] If any confirmation fails, decide whether the advertised guarantee should be softened in the docs until it is fixed — an unverified claim in README or docs-site is worse than an absent one. The one failure, R6 (`package-channels.yml` a dead gate), was fixed by `pose-package-channel-delivery`, and the v5.0.1 release run published the WinGet and Homebrew manifests, so there is no claim left to soften. (owner:@pose-maintainers crit:medium review:2026-09-18)
