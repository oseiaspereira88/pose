...
- [covered: pose-dependency-pin-refresh] Decided: Dependabot, not a warning check — a check that reports a stale pin only moves the manual edit that already was not happening. Original item: digest pins do not refresh themselves: upstream security fixes now arrive only when someone updates a SHA by hand, and nothing reports that a pin has fallen behind. Decide between Dependabot on the actions and Docker ecosystems, or a check that compares each pin against its tag's current target and warns. This is the cost this spec deliberately accepted and it should not stay unmanaged. (owner:@pose-maintainers crit:medium review:2026-10-02)
