...
- [done] Ship `changelog.json` with an empty `adopted_at` rather than this repository's own date, which every instance currently inherits — same family as the self-referential policy roots of issue #17. Done in `pose-changelog-adoption-is-the-instances`, and it needed three changes rather than one: shipping empty alone broke every fresh install, because `pose check` refused an empty date with `adopted_at is required`. The template ships empty, `pose install`/`pose update` stamp the day the instance received the policy, and an empty date now means the contract is not adopted. — owner:unowned crit:medium review:2026-12-09
