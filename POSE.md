...
- `update` — single-command complete update: automatically checks and updates the `pose` binary executable to the latest GitHub release, refreshes scaffolds, rules, workflows and MCP config (`--force`), and migrates the repository instance schema (`.pose/schema-version`). A plain update merges `POSE.md`/`AGENTS.md` (instance-owned sections keep their content; an instance edit a merge cannot keep is saved to `<file>.pose-backup`, and a section that still matches what POSE last wrote is simply replaced), delivers machinery — backing up only files the instance edited since POSE delivered them, as recorded by digest in `.pose/state/machinery-manifest.json` — and seeds absent instance config; `--force` additionally reruns `install` over the instance, which resets the manuals wholesale. Use `--dry-run` to preview or `--no-self` to skip the binary self-update. A run that delivered its files but finds the instance's own state invalid reports that through the final gate and does not roll back. Downgrade is always refused.
...
- `review bundle` / `review attest` / `review auto-attest` / `review verify` / `review record` — opt-in fixed-point review (`review_bundles: true` plus an adoption date). Evaluation is a DAG, and each stage names the one before it: the implementation subject carries an `implementation_digest` over its classified content alone — not over change-set ids or refs, so a squash or rebase of the same content keeps its identity — and an observer of the implementation anchors there rather than on the bundle that will contain it. Sealed evidence records `subject_observation` as `observed`, `carried-forward` or `unknown`; a result naming no commit is unknown, never current. A change set's range observation reports how many commits `base..head` spans against how many it attributes, because paths resolved from trailers are attributed while the range between them is only provenance; it sits outside the sealed payload, so an unrelated commit never stales a review of content that did not move. A criterion is `mechanical` when it names evidence classes a registered check emits, and `judgment` otherwise: `auto-attest` answers the first kind from the sealed evidence and reports the second as pending, refusing `--apply` until a reviewer records a conclusion for each. A profile may declare `kind` explicitly to ask for judgment on a criterion a check also covers; it may not declare `mechanical` on one naming no evidence class. A component-scoped tool whose component the validation matrix declares runs no check (`replaceDefaultChecks` with an empty list) is planned with `producer_coverage: none` and may be disposed `not-used` with a reason, because nothing in the repository can emit what it asks for; everywhere else a required tool must still pass. Preparation hashes the semantic Intent, Requirements, Technical Plan and Decisions, attributed patch/tree content, consumed plan inputs, validation identities and child bundles. Tasks, execution logs, Final Report, lifecycle metadata, attestations and derived state remain outside the review digest but retain their own gates. `--seal` persists immutable JSON in the review-bundles directory under an `rvb-` identity; attestations are separate append-only JSON in review-attestations under an `rva-` identity. `auto-attest` extracts matching evidence from validation results and resolves tool dispositions deterministically. Semantic or source changes create a superseding bundle and typed delta; derived closeout changes do not cause rereview. Local review stays offline. Conductor may return an optional Ed25519 envelope only when its issuer/public-key pin is trusted by policy. Human plan output groups repeated path warnings and separates active required, recommended and completion-deferred tools; JSON retains full provenance. Sealing also records the governance contracts in force (`component-aware`, `review-bundles`, `evidence-vocabulary`) and the gates that decide the bundle (reservations, accepted-risk severities, criterion reuse); verification reads them from the bundle, and only `require_signed_attestations` is read live. Sealing warns about evidence produced against a commit other than the approved head. An attestation's `passed` criterion must cite evidence the bundle contains, of a class the criterion accepts (any one of its `evidence_classes`), from the component it is about — evidence from a directory inside a component does not answer for the component. `--criterion ID|disposition|evidence|rationale` records `passed`, `not-applicable` (rationale required) or `finding` (names a recorded finding); `--finding ID|severity|disposition|action|evidence[|owner|rationale|review-by]` needs a severity and an action, and `accepted-risk` needs a sealed accepted severity, an owner, a rationale and a review date. Read-only MCP: `pose_review_bundle`.
...
- **Output channels** — the result (verdict, findings, data) goes to stdout and progress (steps, usage, the command's own failures) to stderr, so `pose x --json | jq` is safe and `2>/dev/null` never hides a finding. `--json` prints one document whose finding `severity` is the untranslated anchor; `--quiet` prints the verdict alone; `pose validate --verbose` streams a check's output instead of capturing it. Colour, symbols and the spinner exist only on a terminal, resolved per stream, and honour `--color`, `POSE_COLOR`, `NO_COLOR` and `TERM`; `COLUMNS` sets the wrap width. Exit codes: 0 pass, 1 gate failure, 2 usage error.
- `artifact-check` — parses exact `### Artifacts` actions, resolves an explicit base/head range or `POSE-Spec: <slug>` commit trailers with safe structured Git arguments, and reports resolvability, existence, action mismatch, undeclared and orphan findings. A claim on a changelog fragment that `pose release prepare` archived resolves through the release manifest, which records the fragment's spec and digest; prepare never edits a spec. Commits modifying artifacts declared by a spec must carry a `POSE-Spec: <slug>` trailer in their commit message for `artifact-check` and `pose close` to attribute the changes. `pose report --change-from/--change-to` persists immutable change-set evidence; `pose index` projects claims, observations, reverse provenance and findings into `delivery-integrity.json`. MCP: `pose_delivery_integrity`.
...
- `stats governance` — read-only projection over report history, sealed review bundles and attestations. It keeps preparation, judgment, intervention, freshness, telemetry and coverage as independent dimensions, never emits a quality score, and reports missing/invalid data explicitly. The MCP equivalent is `pose_governance_stats`; remediation lineage remains unavailable until its explicit event contract is adopted.
- `assess design --spec <slug>` — read-only bounded projection over the canonical sealed review subject. It observes dependency, component, delivery-metadata, governance-contract and public-contract deltas, keeps unknown/unsupported coverage explicit, and never computes a complexity or architecture score. The MCP equivalent is `pose_design_delta`.
...
This repository has enabled POSE Contributor Mode. Executing AI agents collaborate on improving POSE engine stability and developer experience under explicit user adjudication.

- **Continuous Opportunity Scouting**: On every issue resolution or task execution, proactively scout for related improvements, edge cases, root-level parity gaps, or complementary corrections across adjacent tools and commands.
- **User Consent & Explicit Adjudication**: Staging or submitting contributions upstream is ALWAYS an explicit user decision. Agents must ask the user for confirmation before staging drafts under `.pose/contributions/` or opening GitHub issues.
- **Lifecycle & Status Integrity**: Track contribution states (`staged` -> `submitted` / `dismissed`) with `pose contribute list --status <staged|submitted|dismissed|all>`, `pose contribute submit <file>`, `pose contribute mark-submitted <file> [--issue-url <url>]`, or `pose contribute dismiss <file>`.
