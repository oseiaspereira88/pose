...
- [`.pose/state/project-state.md`](.pose/state/project-state.md): native project-state artifact (`pose state`/`pose_project_state`); staleness policy in [`.pose/policy/state.json`](.pose/policy/state.json) (optional, defaults 7 days / 20 commits).
...
pose state [init|refresh|diff]     # native project-state artifact; no subcommand = validate
...
- `state` (spec `pose-project-state-artifact`) — native `.pose/state/project-state.md` artifact: answers "what is the current state of this project?" in one read instead of scanning specs/roadmaps/follow-ups/capabilities/knowledge/reports every session. `curated` sections (executive summary, current direction — human prose, preserved verbatim) and `derived` sections (specs & roadmaps, follow-ups, capabilities, decisions & knowledge, validation & evidence, architecture — counts and typed pointers, never copied content). `init` scaffolds; `refresh` recomputes derived sections while preserving curated ones, stamping `generated_at`/`baseline_commit`; with no subcommand, validates schema, staleness (age/commits since the last refresh, policy in [`.pose/policy/state.json`](.pose/policy/state.json)) and **per-section hash** — a hand-edited derived section fails nominally (`[TAMPERED]`). `diff` compares the last two refreshes. MCP equivalent: `pose_project_state` (`section` param fetches just one). Additive: a project without `.pose/state/` stays valid everywhere. The Architecture section reports `unavailable` in this version — no local GraphForge export producer exists yet.
