...
var commandHelpCatalog = map[string]CommandHelp{
	...
	"review": {
		...
		Usage:           "pose review <bundle|auto-attest|attest|verify|record> <scope|xref> [options]",
		...
		Subcommands: []SubcommandHelp{
			...
			{"attest", "pose review attest <bundle-id|scope|xref> --reviewer <id> --decision <decision> --evidence <ref> [--criterion ID|disposition|evidence|rationale] [--mapping CRITERION|DELTA|basis-or-disposition|rationale] [--apply] [--expect-context <digest>]", "Record a manual review decision attestation; cross-project writes may target a qualified xref scope only with a fresh context digest", "Registra uma atestação manual; escritas externas podem usar escopo xref qualificado somente com digest de contexto atual"},
