...
var commandHelpCatalog = map[string]CommandHelp{
	...
	"review": {
		...
		Subcommands: []SubcommandHelp{
			...
			{"attest", "pose review attest <bundle-id> --reviewer <id> --decision <decision> --evidence <ref> [--criterion ID|disposition|evidence|rationale] [--mapping CRITERION|DELTA|basis-or-disposition|rationale] [--apply]", "Record a manual review decision attestation; --criterion overrides one criterion, e.g. ID|not-applicable||<why>; --mapping answers one observed structural fact, e.g. design-causality|SD-1a2b3c4d|D2 or |accepted-risk|<why>", "Registra manualmente uma atestação de decisão de review; --criterion sobrepõe um critério, ex. ID|not-applicable||<motivo>; --mapping responde um fato estrutural observado, ex. design-causality|SD-1a2b3c4d|D2 ou |accepted-risk|<motivo>"},
	...
	"state": {
		...
		Usage:           "pose state [init|refresh [--if-stale]|diff]",
