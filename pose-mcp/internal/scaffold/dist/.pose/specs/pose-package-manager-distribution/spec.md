...
- [spawned: pose-package-channel-delivery] Confirm the first real `package-channels.yml` clean-host run on the first tagged release (brew/winget unavailable in this sandbox). Attempted on v0.18.2 and found the gate has never run (trigger never fires) and fails immediately when dispatched (`go run` from a non-module root). Tracked as F1+F2 on `pose-first-release-evidence-confirmation`. (owner:@pose-maintainers crit:medium review:2026-08-19)
- [open] Submit the first generated WinGet manifest to `winget-pkgs` and record the observed publication lag in `package-channels.md`. No longer blocked by F1+F2 — `pose-package-channel-delivery` gave the gate a trigger that fires and a command that runs, and `pose-package-channel-workflow-safety` made its checkout safe — but still waiting on the first tagged cut to actually emit a manifest, since neither change has run in a release yet. (owner:@pose-maintainers crit:low review:2026-09-18)
