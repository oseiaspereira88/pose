...
- [done] Review the first CI runs of security.yml and scorecard.yml on this branch's PR, triage findings and record the baseline score. Triaged: security.yml green (govulncheck, codeql, secrets, workflow-contract); OpenSSF baseline 4.2 recorded. The two real findings became follow-ups on `pose-first-release-evidence-confirmation` (F3, F4). (owner:@pose-maintainers crit:high review:2026-08-14)
...
- [spawned: pose-actions-node24-bump] Bump the first-party actions off Node.js 20. Every CI run since 2026-08-07 annotates that `actions/checkout@v4`, `actions/setup-go@v5` and `actions/upload-artifact@v4` target a deprecated runtime and are being forced onto Node.js 24; the workflows pass today only because the runner substitutes it. The forcing is a transition measure, so the deadline is GitHub's, not ours — pin to the majors that declare Node 24 and confirm the annotation is gone. SHA-pinned third-party actions are a separate axis and are unaffected. (owner:@pose-maintainers crit:medium review:2026-10-02)
