...
- [done] Both ran and passed on v0.20.3: the independent verification (run 31212710370) exercised the extension chain end to end against the published artifact — verified against its real Sigstore signature, installed after verification, and a tampered copy rejected — and `pose release status --version v0.20.3` projects `verified`. Every earlier test of that path had substituted a fake for cosign. Original item: confirm on the next cut that the signing step and the consumer-side extension verification both pass against the published artifact — they have never run. (owner:@pose-maintainers crit:high review:2026-09-04)
