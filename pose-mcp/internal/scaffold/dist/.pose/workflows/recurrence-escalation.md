...
Ativar correção sistêmica when houver retrabalho recorrente não coberto pelos workflows atuais.

## Preconditions

- Existe registro de incidentes/retrabalho por período with classificação por domínio e causa.
...
Ative este workflow when qualquer critério abaixo for atendido no período móvel de 30 dias:
...
1. Consolidar evidência da métrica `recurrence_rework_uncovered` with recorte de 30 dias.
2. Confirmar que o padrão não está coberto por workflow vigente e registrar o gap.
3. Criar workflow especializado em `.pose/workflows/<nome>.md` with escopo, precondições, checks e saídas.
4. Vincular o novo workflow às `rules` de domínio correspondentes no próprio arquivo e no `.pose/workflows/review.md` when aplicável.
5. Atualizar `spec` relacionada with justificativa, critérios de aceite e riscos residuais.
...
- `.pose/rules/knowledge-governance.md` (when houver artifacts de conhecimento/processo)

Em conflito, apply a alternativa mais restritiva.
...
- Se `ajustar`/`descartar`, abrir follow-up with owner, prazo e critério de saída.
...
- Resultado da revisão de piloto with decisão final.
