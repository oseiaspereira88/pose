...
- `check` — valida integridade estrutural POSE (paths obrigatórios e referências em `AGENTS.md`/`POSE.md`) **mais** schema de [`.pose/indexes/validation-matrix.json`](.pose/indexes/validation-matrix.json), sync de [`.pose/indexes/task-map.json`](.pose/indexes/task-map.json) e o grafo nativo de dependências entre specs. Falha em `--strict`, vira aviso em `--tolerant`.
...
- `upgrade` — atualização completa em comando único: verifica e atualiza automaticamente o binário executável do `pose` para o release mais recente do GitHub, sincroniza regras, workflows, templates e configurações MCP (`--force`), e migra o schema (`.pose/schema-version`) do repositório. Use `--dry-run` para pré-visualizar ou `--schema-only` para ignorar a atualização do binário. Downgrades são sempre recusados.
...
- `review-check` / `closeout-check` — validam tentativas imutáveis contra perfis versionados e digests canônicos, propagando aprovação e fechamento por `spec:`, `milestone:` e `roadmap:`. `pose review record` só grava com `--apply`; `pose close` aplica a transição após o gate. `continuous-closeout` persiste o escopo terminal e expõe a menor próxima ação sem ampliar autoridade. A projeção MCP read-only é `pose_closeout_state`.
- `artifact-check` — valida ações exatas em `### Artifacts` contra range Git explícito ou trailers `POSE-Spec`, emitindo findings de resolvability, existence, mismatch, undeclared e orphan. `pose report --change-from/--change-to` persiste o change set; `pose index` gera `delivery-integrity.json`. O backfill é dry-run-first e exige `--confirm-spec-edits`. MCP: `pose_delivery_integrity`.
- `surface-check` / `roadmap-check` — estendem o mesmo grafo com refs tipadas `delivers`, entrypoints de produção, `evidenceClass` fechado e resultados estruturados vinculados ao provenance digest. Surfaces exigem reachability mais integration/e2e; capabilities compostas exigem integration. Critérios de roadmap aceitam apenas refs/checks/reports registrados, nunca comandos crus. MCP: `pose_surface_assurance`.
...
- `recurrence-effect` — fecha o ciclo de feedback: `--register` vincula um escalonamento à sua intervenção (`rule:|workflow:|spec:<nome>`) e janela de observação no `interventions.jsonl` append-only; compara taxa de recorrência antes/depois por intervenção. Vereditos `INEFFECTIVE` exigem follow-up governado.
- `extension install|list|remove|verify` — ciclo de vida de extensões assinadas: pacotes são diretórios com `extension.json` e alvos confinados em `.agents/skills/`, `.pose/workflows/`, `.pose/rules/` e `.pose/templates/`. `install`/`remove` exigem `--yes` e são transacionais. Projeção MCP read-only: `pose_extension_list`.
- `skills-check` — gate de conformidade do Agent Skills: valida metadados obrigatórios em `.agents/skills/<slug>/SKILL.md`, links relativos, instruções inseguras e padrões de segredos.
- `state` (spec `pose-project-state-artifact`) — artefato de estado nativo do projeto: consolida em uma leitura seções curadas (prosa humana) e derivadas (specs, roadmaps, follow-ups, capabilities, decisões e validação). `init` cria a estrutura, `refresh` recomputa seções derivadas preservando as curadas, e `diff` compara atualizações. Projeção MCP: `pose_project_state`.
- `stacks [--path dir] [--json]` — inspeção read-only do catálogo de perfis detectados (Node.js, Go, Rust, Java, Python, .NET).
- `assess discover|integrate|tech-debt` — motores de assessment nativo para LOC, débitos, estrutura de componentes e integridade de contratos inter-módulo.
