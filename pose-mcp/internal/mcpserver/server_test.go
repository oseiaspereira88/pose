...
func TestToolsList(t *testing.T) {
	...
	if len(tools) != 53 {
		t.Fatalf("tools = %d, want 53", len(tools))
	}
	...
	for _, want := range []string{"pose_get_spec", "pose_list_specs", "pose_spec_readiness",
		"pose_list_roadmaps", "pose_get_roadmap", "pose_get_changelog", "pose_release_status", "pose_closeout_state", "pose_review_plan", "pose_review_bundle", "pose_federated_roadmap_acceptance", "pose_delivery_integrity",
