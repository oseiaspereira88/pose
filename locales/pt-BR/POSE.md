...
- `review-check` / `closeout-check` — validam tentativas imutáveis de review contra perfis versionados e digests canônicos de escopo, propagando aprovações atuais e o fechamento de filhos por `spec:`, `milestone:` e `roadmap:`. `pose review record` é dry-run por padrão e só anexa nova tentativa com `--apply`; `pose close` aplica apenas transição de ciclo de vida com gate de review. `continuous-closeout` persiste um escopo terminal e informa a menor próxima ação governada até o sucesso terminal, sem conceder autoridade nova. A projeção MCP read-only é `pose_closeout_state`.
- `artifact-check` — interpreta as ações exatas de `### Artifacts`, resolve um range base/head explícito ou trailers `POSE-Spec` com argumentos Git estruturados e seguros, e reporta findings de resolvability, existence, action mismatch, undeclared e orphan. `pose report --change-from/--change-to` persiste evidência imutável de change set; `pose index` projeta claims, observações, proveniência reversa e findings em `delivery-integrity.json`. O `artifact-backfill` é dry-run-first e exige `--confirm-spec-edits` antes de aplicar propostas inequívocas. MCP: `pose_delivery_integrity`.
